...
OCRWidget::OCRWidget(QWidget *parent) : QWidget(parent), ui(new Ui::OCRWidget) {
    ui->setupUi(this);
...
void OCRWidget::init() {
    ...
    qDebug()<<m_settings.readSettings("Lang", "Dir").toString();
    ...
    ui->checkBox->setChecked(
                m_settings.readSettings("OCR", "Adaptive Threshold").toBool());
    ui->lineEdit_blacklist->setText(
                m_settings.readSettings("OCR", "Blacklist").toString());
    ui->lineEdit_whitelist->setText(
                m_settings.readSettings("OCR", "Whitelist").toString());
...
void OCRWidget::initPSMModel() {
    ...
    m_mappsm->insert("Assume single column of text",
    ...
    m_mappsm->insert("Assume a single uniform block of text",
    ...
    m_mappsm->insert("Assume a single word in a circle",
                     tesseract::PSM_CIRCLE_WORD);
    m_mappsm->insert("Sparse text in no particular order",
    ...
    ui->comboBox_PSM->setCurrentText(
                m_settings.readSettings("OCR", "PSM Mode").toString());
}
...
void OCRWidget::clearResult() { result.clear(); }
...
void OCRWidget::initSelLangs() {
    ...
    for (auto &index : selLang) {
        for (auto &data : m_mdllng->findItems(index, Qt::MatchExactly, 1)) {
            ui->treeView->selectionModel()->select(data->index(),
...
void OCRWidget::setImage(QImage img) { *m_ocrImg = img; }

void OCRWidget::setImageList(QList<QImage> &_images) { m_images = &_images; }
    ...
    if (tess->Init(directory.toStdString().c_str(),
        ...
        for (auto &var : *m_images) {
            ...
            if (ui->checkBox->isChecked()) {
                ...
                *m_ocrImg = var;
            tess->SetImage(m_ocrImg->bits(), m_ocrImg->width(), m_ocrImg->height(), 4,
        ...
        //    Pixa **ttt = nullptr;
        //    Boxa *boxes;
        //    boxes = tess->GetComponentImages(tesseract::RIL_BLOCK, true, ttt,
        //    nullptr); m_pen2.setColor(Qt::red); m_pen2.setWidth(2); m_ocrrect =
        //    new QRectF(); for (int32_t i = 0; i < boxes->n; i++) {
...
int OCRWidget::getConfidence() { return confidence; }

QList<QImage> OCRWidget::getImageList() { return *m_images; }

QString OCRWidget::getResult() const { return result; }
...
//    m_settings.writeSettings("Lang", "Dir", directory);
    ...
    m_settings.writeSettings("OCR", "Adaptive Threshold",
