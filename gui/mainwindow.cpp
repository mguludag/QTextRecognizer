...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(ui->actionCancel, SIGNAL(triggered()), this,
          SLOT(cancelPreprocess()));
  connect(m_rwidget, SIGNAL(angleChanged()), this, SLOT(rotatePixmap()));
  connect(m_adjWidget->getBrightness(), SIGNAL(sliderReleased()), this,
          SLOT(setBrightness()));
  connect(m_adjWidget->getContrast(), SIGNAL(sliderReleased()), this,
          SLOT(setContrast()));
  connect(m_adjWidget, SIGNAL(accepted()), this, SLOT(acceptAdjust()));
  connect(m_adjWidget, SIGNAL(rejected()), this, SLOT(rejectAdjust()));
  connect(m_adjWidget->getThreshold(), SIGNAL(sliderReleased()), this,
          SLOT(setMThreshold()));
  connect(m_adjWidget, SIGNAL(adaptiveThresOK()), this, SLOT(setAThreshold()));
  connect(m_adjWidget, SIGNAL(adaptiveThresCncl()), this,
          SLOT(setAThreshold()));
...
//  ui->graphicsView->setViewport(gl);
  ...
  ui->graphicsView->setDragMode(QGraphicsView::RubberBandDrag);
   ...
   restoreGeometry(m_settings.readSettings("Window",
...
void MainWindow::mouseReleaseEvent(QMouseEvent *) { hidePopups(); }
...
void MainWindow::moveEvent(QMoveEvent *) {
  ...
  m_rwidget->move(this->geometry().x() + ROTATE_POPUP_X,
                  this->geometry().y() + POPUP_Y);
  m_adjWidget->move(this->geometry().x() + ADJ_POPUP_X,
                    this->geometry().y() + POPUP_Y);
  m_ocrWidget->move(this->geometry().x() + OCR_POPUP_X,
                    this->geometry().y() + POPUP_Y);
  m_settwidget->move(this->geometry().right() + SETT_POPUP_X,
                     this->geometry().y() + POPUP_Y);
}
...
void MainWindow::openImage() {
  ...
  if (filename.length() > 0) {
    ...
    m_pixmapItem->setTransformOriginPoint(
        m_pixmapItem->boundingRect().center());
...
void MainWindow::showPreprocess() {
  ...
  ui->label->setText("You can drag-drop the corners and apply transformation "
    ...
    *m_imgMat = ConvertImage::qimageToMatRef(*m_imgQ, CV_8UC4);
    ...
    if (m_pointStd.size() > 3) {
      for (auto i : m_pointStd) {
        m_pointQ.push_back(QPointF(static_cast<double>(i.x) / 1.92,
                                   static_cast<double>(i.y) / 1.92));
      }
...
void MainWindow::applyPreprocess() {
  ...
  if (!cancelpreprocess) {
    ...
    *m_imgQ = getImage(*m_scene, m_pixmapItem->sceneBoundingRect());
    ...
    for (auto i : m_pointQ) {
      m_pointStd.push_back(
          cv::Point2f(static_cast<float>(i.x()), static_cast<float>(i.y())));
    }
...
void MainWindow::autoDetectLayout() {
  ...
  if (it && !it->Empty(tesseract::RIL_BLOCK)) {
    do {
      ...
      if (width > 10 && height > 10) {
        rects.append(QRectF(x1 - 1, y1 - 1, static_cast<double>(width),
  ...
  if (std::abs(avgDeskew) > 0.1) {
    ...
    for (int i = 0, n = rects.size(); i < n; ++i) {
      m_rect = new MyRectF(m_scene, rects[i].topLeft(), rects[i].bottomRight(),
                           ++rects_size, m_pixmapItem->sceneBoundingRect());
...
void MainWindow::drawPolygon() {
  ...
  m_pts[0]->setRatio(
      static_cast<int>(m_pixmapItem->pixmap().rect().height() * 0.03));
  m_pts[1]->setRatio(
      static_cast<int>(m_pixmapItem->pixmap().rect().height() * 0.03));
  m_pts[2]->setRatio(
      static_cast<int>(m_pixmapItem->pixmap().rect().height() * 0.03));
...
void MainWindow::checkPoints() {
  ...
  if (m_polyf->shape().length() <
      m_pixmapItem->pixmap().rect().height() * 0.05) {
    m_pointQ[0] = m_pixmapItem->sceneBoundingRect().bottomLeft();
    m_pointQ[1] = m_pixmapItem->sceneBoundingRect().topLeft();
    m_pointQ[2] = m_pixmapItem->sceneBoundingRect().topRight();
    m_pointQ[3] = m_pixmapItem->sceneBoundingRect().bottomRight();
...
void MainWindow::setBrightness() {
  ...
  mt_cont = false;
  *m_imgQ_copy =
      ConvertImage::matToQimageRef(*m_imgMat, QImage::Format_RGBA8888);
...
void MainWindow::showResult() {
  ...
  onprogress = false;
  ui->label->setText("OCR finished in " + QString::number(m_etimer->elapsed()) +
                     " msecs | Avg. word confidence is " +
                     QString::number(m_ocrWidget->getConfidence()) + "%\t");
...
void MainWindow::writePos() {
  ui->label_2->setText(
      "(" + QString::number(ui->graphicsView->getProps().m_pos.x()) + ", " +
...
void MainWindow::endRect() {
  ...
  if (ui->graphicsView->dragMode() == QGraphicsView::RubberBandDrag &&
      !previewpreproc && m_scene->items().size() > 0) {
    m_rect->setBottomRight(ui->graphicsView->mapToScene(
        ui->graphicsView->getProps().m_bottomright));
    if (ui->graphicsView->getProps().m_topleft !=
        ui->graphicsView->getProps().m_bottomright) {
      m_scene->addItem(m_rect);
      m_scene->items().first()->setData(0, "rect");
      m_scene->items().first()->setData(1, m_rect->getRect());
      m_scene->items().first()->setData(2, rects_size);
      ui->label->setText("You are selecting the area for recognize. Press "
...
QPointF MainWindow::getRotatedPoint(QPointF p, QPointF center,
                                    qreal angleRads) {
...
QRectF MainWindow::getMinimumBoundingRect(QRectF r, qreal angleRads) {
  ...
  qreal minX = qMin(topLeft.x(),
                    qMin(bottomRight.x(), qMin(topRight.x(), bottomLeft.x())));
  qreal minY = qMin(topLeft.y(),
                    qMin(bottomRight.y(), qMin(topRight.y(), bottomLeft.y())));

  qreal maxX = qMax(topLeft.x(),
                    qMax(bottomRight.x(), qMax(topRight.x(), bottomLeft.x())));
  qreal maxY = qMax(topLeft.y(),
                    qMax(bottomRight.y(), qMax(topRight.y(), bottomLeft.y())));
...
QRectF MainWindow::getRotatedRect(QRectF _rect) {
  ...
  QRectF rotatedRect(
      getRotatedPoint(_rect.topLeft(), _rect.center(), degToRad(m_angle)),
      _rect.size());
  return rotatedRect;
}
...
void MainWindow::on_actionOpenImage_triggered() {
  ...
  filename =
      QFileDialog::getOpenFileName(this, "Please Select The Image", "",
...
void MainWindow::on_actionBest_Fit_triggered() { centerImage(); }

void MainWindow::on_actionZoom_In_triggered() { z->gentle_zoom(1.10); }

void MainWindow::on_actionZoom_Out_triggered() { z->gentle_zoom(0.90); }
...
void MainWindow::on_actionSave_triggered() {
  ...
  QString nameFile = QFileDialog::getSaveFileName(
      this, tr("Save text to file"), "", tr("Text Document (*.txt);"));
  if (nameFile != "") {
    QFile file(nameFile);
